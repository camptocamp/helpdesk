...
class TestHelpdeskTicketEquipment(TransactionCase):
    ...
    def test_01_equipment_assignment_valid_location(self):
        """Test that equipment can be assigned when location matches"""
        self.ticket.equipment_id = self.equipment
        self.assertEqual(self.ticket.equipment_id, self.equipment)
        self.assertTrue(self.ticket._is_valid_equipment_location())

    def test_02_equipment_assignment_no_location(self):
        """Test that equipment without location can be assigned to any ticket"""
        self.ticket.equipment_id = self.equipment_no_location
        self.assertEqual(self.ticket.equipment_id, self.equipment_no_location)
        self.assertTrue(self.ticket._is_valid_equipment_location())

    def test_03_equipment_assignment_invalid_location(self):
